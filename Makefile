...
test: cleanup
	$(QUIET)$(COVERAGE) run -p --source=multicast* -m unittest discover --verbose -s ./tests -t ./ tests 2>/dev/null || $(PYTHON) -m unittest discover --verbose -s ./tests -t ./ tests || DO_FAIL="exit 2" ;
